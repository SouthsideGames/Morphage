...
namespace Mutagen
{
    ...
    public class Enemy
    {
        ...
        public float slowT, slowMul = 1f; // generic movement slow (Frost Breath, Sonic Screech)
        ...
        public Enemy() { }
        public Enemy(EnemyDef cfg, float x, float y, float waveScale, string elite = null) => Set(cfg, x, y, waveScale, elite);

        /// <summary>(Re)initialise all state — used for fresh spawns AND pooled reuse, so it must reset
        /// every field (incl. field-initialised ones like slowMul, which don't re-run on reuse).</summary>
        public Enemy Set(EnemyDef cfg, float x, float y, float waveScale, string elite = null)
        {
        ...
        public void Hurt(float amount, Game game, float kx = 0f, float ky = 0f, bool silent = false)
        {
            ...
            if (pl != null)
            {
                // Synergies — conditional damage amps keyed on build + target state (see Game.ActiveSynergies).
                bool slowed = slowT > 0f, poisoned = poisonT > 0f;
                float lowHp = hp / maxHp;
                float syn = 1f;
                if (slowed && pl.HasMove("frost")) syn *= 1.25f;                                            // Brittle
                if (poisoned && pl.Has("toxicskin")) syn *= 1.15f;                                          // Plague
                if (slowed && poisoned && pl.HasMove("frost") && pl.HasMove("venom")) syn *= 1.20f;         // Cryotoxin
                if (poisoned && pl.HasMove("firebreath") && pl.HasMove("venom")) syn *= 1.18f;              // Wildfire
                if (lowHp < 0.30f && pl.Has("crit") && pl.Has("carnivore")) syn *= 1.35f;                   // Executioner
                if (lowHp > 0.90f && pl.HasMove("charge") && pl.Has("power")) syn *= 1.25f;                 // Ambush
                if (cfg.boss && pl.Has("bulk") && (pl.HasMove("quake") || pl.HasMove("gravity"))) syn *= 1.20f; // Giant Slayer
                amount *= Mathf.Min(syn, 2.2f);                                                             // clamp runaway stacking
            }
            hp -= amount; hitFlash = 0.12f; game.stats.damageDealt += amount;
            if (pl != null && pl.lifesteal > 0f) pl.Heal(amount * pl.lifesteal);        // Vampirism
            if (kx != 0f || ky != 0f) { x += kx; y += ky; }
            if (!silent)
            {
                bool big = crit || amount >= 28f;
                game.AddFloater(x, y + r, Fx.Num(amount),
        ...
        public void Die(Game game)
        {
            ...
            if (cfg.boss) Vfx.Spawn("BossExplosion", x, y, 64f);
            else if (elite != null || r >= 16f) Vfx.Spawn("Explosion", x, y, r * 1.1f);
        ...
        {
            attackType = Rng.Next() < 0.5f ? "spread" : "charge"; // seeded so both clients agree
            windup = 0.8f;
        ...
        public void Update(float dt, Game game)
        {
            ...
            if (poisonT > 0f)
            {
                ...
                {
                    poisonFloat = 0.45f;
                    game.AddFloater(x + Fx.Rand(-8f, 8f), y + r, Fx.Num(poison * 0.45f), Palette.PoisonFloat, 12f);
                }
